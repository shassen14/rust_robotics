...
impl Model {
    ...
    fn calculate_a(
        ...
        // where
        // f0: x_dot = vel_x * cos(yaw)
        // f1: y_dot = vel_x * sin(yaw)
        // f2: yaw_dot = vel_x * tan(road_wheel_angle) / length_total

        let df0x0 = 0f64;
        let df0x1 = 0f64;
        let df0x2 = -u[0] * f64::sin(x[2]);

        let df1x0 = 0f64;
        let df1x1 = 0f64;
        let df1x2 = u[0] * f64::cos(x[2]);
    ...
    fn calculate_b(
        ...
        // f1: y_dot = vel_x * sin(yaw)
        // f2: yaw_dot = vel_x * tan(road_wheel_angle) / length_total
        let df0u0 = f64::cos(x[2]);
        let df1u0 = f64::sin(x[2]);
        let df2u0 = f64::tan(u[1]) / length_total;

        let df0u1 = 0.;
        let df1u1 = 0.;
        let df2u1 = u[0] / (f64::cos(u[1]) * f64::cos(u[1]) * length_total);
...
impl base::System<f64, 3, 2> for Model {
    fn get_derivatives(
        ...
        // x_dot = vel_x * cos(yaw)
        // y_dot = vel_x * sin(yaw)
        // yaw_dot = vel_x * tan(road_wheel_angle) / length_total
        let pos_x_dot: f64 = u[0] * f64::cos(x[2]);
        let pos_y_dot: f64 = u[0] * f64::sin(x[2]);
        let yaw_dot: f64 = u[0] * f64::tan(u[1]) / self.get_wheelbase();
